...
import { CalendarDays, CalendarPlus, Download, FileUp, GraduationCap, Heart, LayoutDashboard, RefreshCw, Users } from "lucide-react";
    ...
    <>
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col border-r border-slate-200 bg-white px-4 py-5 md:flex">
        ...
        <div className="mt-auto space-y-3 rounded-2xl bg-slate-50 p-3">
          ...
          <div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => setImportOpen(true)} className="rounded-xl bg-white px-2 py-2.5 text-center text-xs font-bold text-slate-700 shadow-sm ring-1 ring-slate-200"><FileUp className="mx-auto mb-1 h-4 w-4 text-emerald-600" />Import syllabus</button><button type="button" onClick={() => void exportData()} disabled={exportBusy} className="rounded-xl bg-white px-2 py-2.5 text-center text-xs font-bold text-slate-700 shadow-sm ring-1 ring-slate-200 disabled:opacity-50"><Download className="mx-auto mb-1 h-4 w-4 text-indigo-600" />{exportBusy ? "Exporting…" : "Export data"}</button></div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-8 border-t border-slate-200 bg-white/95 px-1 pb-[max(env(safe-area-inset-bottom),0.35rem)] pt-1.5 backdrop-blur md:hidden" aria-label="Mobile navigation">
        ...
        <button type="button" onClick={() => setImportOpen(true)} className="flex min-h-14 flex-col items-center justify-center gap-1 rounded-xl text-[10px] font-semibold text-emerald-700"><FileUp className="h-5 w-5" />Import</button>
      ...
      {importOpen && <SyllabusImporter open onClose={() => setImportOpen(false)} onImported={() => router.refresh()} onUndone={() => router.refresh()} />}
    </>
  );
}
